...
class Pipeline(object):
    ...
    def _execute(self, augmentor_image, save_to_disk=True, multi_threaded=True):
    	...
    	for operation in self.operations:
    		images_bis = operation.perform_operation([Image.open(augmentor_image.image_path)])
    		file_name = str(uuid.uuid4())
    		try:
    			for i in range(len(images_bis)):
    				if i == 0:
    					save_name = ((os.path.splitext(os.path.basename(augmentor_image.image_path))[0] +
    								 '_' + str(operation)) +
    								 '.'+ (self.save_format if self.save_format else augmentor_image.file_format))
    
    					images_bis[i].save(os.path.join(augmentor_image.output_directory, save_name))
